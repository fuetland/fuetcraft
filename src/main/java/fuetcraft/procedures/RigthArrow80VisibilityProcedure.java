...
public class RigthArrow80VisibilityProcedure {
	public static boolean execute(LevelAccessor world, double x, double y, double z) {
		if (getBlockNBTLogic(world, BlockPos.containing(x, y, z), FuetcraftModVariables.MapVariables.get(world).processIsActive)) {
			if (getBlockNBTNumber(world, BlockPos.containing(x, y, z), FuetcraftModVariables.MapVariables.get(world).processTimer) < 20
					&& getBlockNBTNumber(world, BlockPos.containing(x, y, z), FuetcraftModVariables.MapVariables.get(world).processTimer) >= 2) {
				return true;
			}
		}
		return false;
